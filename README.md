...
* **Flexibility:** Styles can be applied via `userChrome.css` (UI) and `userContent.css` (web content).
...
* To achieve the native blur effect on Linux, it is recommended to use a window compositor with *blur* support (such as `picom` or the native window manager in Cinnamon or other distributions).

## Contributions
Pull requests are welcome. If you have specific settings for other distributions or environments, please feel free to share them.
